...
class bot:
    ...
    def on_message(self, msg):
        if msg['e'] != 'error':
            ...
            try:
                self.conn.execute('''CREATE TABLE REALF
                    (TIME TEXT  NOT NULL,
                    COIN           TEXT    NOT NULL,
    ...
    def canSell(self, currentTime, currentPrice):
        ...
        try:
            ...
            if(round(df['rsi5o'][498]) >= 80 or round(df['rsi5h'][498]) >= 80 or round(df['rsi5l'][498]) >= 80 or round(df['rsi5c'][498]) >= 80):
    ...
    def buy(self, price, time):
        global no_of_trades
        no_of_trades += 1
        self.conn.execute("INSERT INTO REALF VALUES(?,?,?,'buy',?);", (str(time),self.coin,price,None))
        self.conn.commit()

        self.stoploss = price - price* 10/100
    ...
    def sell(self, price, time):
        ...
        print(msg2)
        self.conn.execute("INSERT INTO REALF VALUES(?,?,?,'sell',?);", (str(time),self.coin,price,round((self.currentAmount - self.previousAmount) * 100/self.previousAmount, 2)))
        self.conn.commit()
        
        url = 'https://api.telegram.org/bot2072407643:AAEq8g8QuQe13WY1n9lhTKh5AWQNClu0w48/sendMessage?chat_id={}&text={}'.format(chatId,msg1+"\n"+msg2)
        requests.get(url)

        initialTradeAmount[self.num] = int(initialTradeAmount[self.num] + self.currentAmount)
        profitperday += profit
        ...
        try:
            ...
            if(getAvg(df, length) > 0.5 ):
                ...
                if((canBuy or temp)
                    ...
                    or (df['rsic'][length-3] < 30 and df['rsi2c'][length-3] < 5 and df['rsic'][length-3] < df['rsic'][length-2])
                    ):
