...
class bot:
    def __init__(self, coin,num):
        ...
        if((datetime.datetime.now().hour*60 + datetime.datetime.now().minute)%240 < 120):
            self.currentAmount = int(initialTradeAmount[num])
            self.previousAmount = int(initialTradeAmount[num])
        else:
            self.currentAmount = int(initialTradeAmount[num] * 2/3)
            self.previousAmount = int(initialTradeAmount[num] *2/3)
    ...
    def on_message(self, msg):
        if msg['e'] != 'error':
            ...
            if(self.previousTime != None and currentTime != self.previousTime):
                ...
                if(currentTime.minute != self.previousTime.minute and currentTime.minute % 15 == 0):
    ...
    def buy(self, price, time):
        ...
        self.conn.commit()
        self.stoploss = price -price *5/ 100
...
#        self.previousAmount = self.currentAmount
        ...
        print(order)
        orders = client.get_all_orders(symbol=self.coin,limit=len(order['fills']))

        self.currentCrypto = float(order['executedQty'])
        self.previousAmount = float(order['cummulativeQuoteQty'])
        initialTradeAmount[self.num] -= self.previousAmount

        msg = "\nBUY : {} : {} : current price {}$, bought for : {}$".format(self.coin, time, price, round(self.previousAmount, 2))
        print(msg)
    ...
    def sell(self, price, time):
        ...
        # orders = client.get_all_orders(symbol=self.coin,limit=len(order['fills']))
        # for i in orders
        self.currentAmount = float(order['cummulativeQuoteQty'])
        
        try:
            client.transfer_dust(asset=self.coin)
        except:
            pass
        profit = self.currentAmount-self.previousAmount
...
def HA(df, length):
    ...
    df['rsi5o'] = RSIIndicator(df['hopen'], window=7).rsi()
    df['rsi5h'] = RSIIndicator(df['hhigh'], window=7).rsi()
    df['rsi5l'] = RSIIndicator(df['hlow'], window=7).rsi()
    df['rsi5c'] = RSIIndicator(df['hclose'], window=7).rsi()
...
def gulian(df, length,coin):
    df = HA(df, length)
    buying1,l3_0Arr = ANN(coin,0)
#    buying2,l2 = ANN(coin,1)

    canBuyIn4 = True
    canBuy = buying1[49] == True #and buying1[48] == False
    if(buying1[49]):
        if(l3_0Arr[47] < 0.05):
            canBuyIn4 = True
        else:
            canBuyIn4 = False
    if(canBuy):
        print(coin,canBuyIn4)
    return df,canBuy,canBuyIn4
...
def collectData(coin, length,temp=False):
    ...
    try:
        ...
        if(getAvg(df, length) > 0.5 ):
            df,canBuy,canBuyIn4 = gulian(df, length,coin)
            if(canBuy and (canBuyIn4 or (datetime.datetime.now().hour*60 + datetime.datetime.now().minute )%240 < 120)
                and df['rsi'][length-2] < 30 and df['rsi2o'][length-2] < 5 and df['rsi'][length-2] < df['rsi'][length-1] 
                and (round(df['rsi5o'][499]) <= 50 and round(df['rsi5h'][499]) <= 50 and round(df['rsi5l'][499]) <= 50 and round(df['rsi5c'][499]) <= 50)):
    ...
    if canBuy:
        try:
            ...
            for product in prods:
                ...
                if(True):
                    if(float(product['c']) * float(product['cs']) < 200000000):
                        break
                        ...
                        if(canBuy):
                            ...
                            if(product['s'] not in chosenCoins):
                                try:
                                    ...
                                    bot(product['s'],chosenCoins.index(product['s']))
                                except:
                                    pass
...
def balanceAmt(num,profit):
    global initialTradeAmount
    for i in range(no_of_coins):
        if(num != i):
            if(profit > 0):
                initialTradeAmount[i] += int(profit/no_of_coins)
                initialTradeAmount[num] -= int(profit/no_of_coins)
            else:
                if(initialTradeAmount[i]!=0):
                    initialTradeAmount[i] += int(profit/no_of_coins)
                    initialTradeAmount[num] -= int(profit/no_of_coins)
...
# orders = client.get_all_orders(symbol="AXSUSDT",limit=1)
# for i in orders:
#     print(i)
