...
class bot:
    ...
    def on_message(self, msg):
        if msg['e'] != 'error':
            ...
            try:
                self.conn.execute('''CREATE TABLE TESTF2
                    (TIME TEXT  NOT NULL,
                    COIN           TEXT    NOT NULL,
    ...
    def canSell(self, currentTime, currentPrice):
        ...
        try:
            ...
            df = HA(df,500)
            # print(df['rsi5o'])
            if(round(df['rsi5o'][499]) >= 85 or round(df['rsi5h'][499]) >= 85 or round(df['rsi5l'][499]) >= 85 or round(df['rsi5c'][499]) >= 85):
                return True
        except:
            pass
    ...
    def buy(self, price, time):
        global tradeAmount
        self.conn.execute("INSERT INTO TESTF2 VALUES(?,?,?,'buy',?);", (str(time),self.coin,price,None))
        self.conn.commit()
        self.stoploss = price -price /2

        
        

        if((datetime.datetime.now().hour*60 + datetime.datetime.now().minute)%240 < 120):
            pass
        else:
            self.currentAmount /= 2

        msg = "\nBUY : {} : {} : current price {}$, bought for : {}$".format(self.coin, time, price, round(self.currentAmount, 2))
        print(msg)
        url = 'https://api.telegram.org/bot2072407643:AAEq8g8QuQe13WY1n9lhTKh5AWQNClu0w48/sendMessage?chat_id=-1001590033171&text={}'.format(msg)
        requests.get(url)
    ...
    def sell(self, price, time):
        ...
        global tradeFinishCount, mainAmount, tradeAmount, initialTradeAmount,profitperday,hasProfitPerDayPosted

        if(self.selfAtStopLoss):
            self.currentAmount = self.previousPrice * (1-2/100) * self.currentCrypto
        else:
        ...
        print(msg2)
        self.conn.execute("INSERT INTO TESTF2 VALUES(?,?,?,'sell',?);", (str(time),self.coin,price,round((self.currentAmount - self.previousAmount) * 100/self.previousAmount, 2)))
        self.conn.commit()
        
        url = 'https://api.telegram.org/bot2072407643:AAEq8g8QuQe13WY1n9lhTKh5AWQNClu0w48/sendMessage?chat_id=-1001590033171&text={}'.format(msg1+"\n"+msg2)
        requests.get(url)
        # tradeAmount += self.currentAmount
        profitperday += round(self.currentAmount-self.previousAmount, 2)

        if(datetime.datetime.now().hour > 6):
            if(not hasProfitPerDayPosted):
                url = 'https://api.telegram.org/bot2072407643:AAEq8g8QuQe13WY1n9lhTKh5AWQNClu0w48/sendMessage?chat_id=-1001590033171&text=Total Profit for today : {}$'.format(profitperday)
                requests.get(url)
        ...
        #     print("########################################################")
        #     print("Main Amount : {}, Trade Amount : {}".format(
        #         mainAmount, tradeAmount))
        #     print("########################################################")
        #     sleep(300)
        #     reStart()
...
def ANN(coin,temp):
    ...
    for i in range(1,length):
        ...
        if(l3_0 > 0.2 and temp == 0):
            buying[i] = True
        elif(l3_0 > 0.2 and temp == 1):
            buying[i] = True
        elif(l3_0<=0.2):
            buying[i] = False
        else:
...
def HA(df, length):
    ...
    df['rsi2h'] = RSIIndicator(df['hclose'], window=2).rsi()
    ...
    df['rsi'] = RSIIndicator(df['hopen']).rsi()
...
def gulian(df, length,coin):
    ...
    canBuy = buying1[49] == True # and buying1[49] == buying2[49]
...
def collectData(coin, length,temp=False):
    ...
    try:
        ...
        if((canBuy or temp)
            and getAvg(df, length) > 0.5 
            and df['rsi'][length-2] < 30 and df['rsi2o'][length-2] < 5 and df['rsi'][length-2] < df['rsi'][length-1] and df['rsi5o'][length-1]<50):
...
def findBestCoin():
    ...
    # canBuy = ((datetime.datetime.now().hour*60 + datetime.datetime.now().minute - (5*60 +30))%240 < 120)
    canBuy = True
        ...
        try:
            ...
            for i in range(len(tempArr)):
                for j in range(len(tempArr)-1):
                    a, b = float(tempArr[j]['volume']) * float(tempArr[j]['lastPrice']), float(
                        tempArr[j+1]['volume']) * float(tempArr[j+1]['lastPrice'])
                    if(a < b):
            ...
            for product in tempArr:
                ...
                if(True):
                    profitPerc = (float(
                        product['lastPrice'])-float(product['openPrice']))*100/float(product['openPrice'])
                    if(float(product['volume']) * float(product['lastPrice']) < 20000000):
                        break
                    if((product['symbol'][len(product['symbol'])-4:] in ("USDT")) and not "DOWN" in product['symbol']
                    and (product['symbol'][:4] not in ("USDC,USDT,BUSD")) and (product['symbol'][:3] not in ("EUR"))):
                        canBuy, df, avg = collectData(product['symbol'], 500)
                        if(canBuy):
                            if(product['symbol'] not in chosenCoins and len(chosenCoins) < no_of_coins):
                                chosenCoins.append(product['symbol'])
                                bot(product['symbol'], df, avg)
...
def reStart():
    global bot, mainAmount, no_of_coins, tradeFinishCount, chosenCoins
    print("Finding coins")
    chosenCoins = []
    tradeFinishCount = 0
    while findBestCoin():
