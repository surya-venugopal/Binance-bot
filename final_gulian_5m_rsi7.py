...
class bot:
    def __init__(self, coin,num):
        ...
        if((datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%240 < 120):
...
def getDiff(coin):
    ...
    for i in range(1,length):
        ...
        if((time.minute + 60*time.hour -330) % 240 == 0):
...
def ANN(coin):
    length = 50
    buying = [False] * length
    percentage = PineActivationFunctionLinear(getDiff(coin))
    l3_0Arr = []
    for i in range(1,length):
...
def collectData(coin, length):
    ...
    try:
        ...
        if(getAvg(df, length) > 0.5 ):
            ...
            if(canBuy and (canBuyIn4 or ((datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%240 < 100 and (datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%240 > 5))
                and df['rsi'][length-2] < 30 and df['rsi'][length-2] < df['rsi'][length-1]
                ): #df['rsi2'][length-2] < 5
...
def findBestCoin():
    ...
    canBuy = ((datetime.datetime.now().hour*60 + datetime.datetime.now().minute - (5*60 +30))%240 < 180)
    # canBuy = True
