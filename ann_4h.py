...
class bot:
    ...
    def canSell(self, currentTime, currentPrice):
        ...
        if(self.target<currentPrice or self.stoploss > currentPrice):
            return True
        # if((datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%60 > 4 and (datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%60 < 11): #and (datetime.datetime.now().hour*60 + datetime.datetime.now().minute -330)%60 < 11
        #     try:
        #         bars = client.get_klines(
        #             symbol=self.coin, interval=Client.KLINE_INTERVAL_5MINUTE, limit=50)
        ...
        #         df,canBuy,canSell = gulian(df, 50,self.coin)
        #         if(canSell):
        #             return True
        #     except:
        #         pass
    ...
    def buy(self, price, time):
        ...
        self.target = price + price*(self.avg * 1.5/100)
...
def getDiff(coin,interval):
    length = 50
    bars = client.get_klines(
        symbol=coin, interval=interval, limit=length)
    df = pd.DataFrame(bars, columns=["time", "open", "high", "low", "close", "volume", "Close time", "Quote asset volume",
                                     "Number of trades", "Taker buy base asset volume", "Taker buy quote asset volume", "Can be ignored"])
    df['open'] = pd.Series(list(map(float, df['open'])))
    df['high'] = pd.Series(list(map(float, df['high'])))
    ...
    if(interval == Client.KLINE_INTERVAL_1HOUR):
        temp = 60
    if(interval == Client.KLINE_INTERVAL_2HOUR):
        temp = 120
    ...
    for i in range(1,length):
        ...
        if((time.minute + 60*time.hour) % temp == 0):
...
def ANN(coin,interval):
    length = 50
    percentage = PineActivationFunctionLinear(getDiff(coin,interval))
    l3Arr = []
    for i in range(1,length):
...
def HA(df, length):
    ...
    df['rsi'] = RSIIndicator(df['hopen'],window=7).rsi()
...
def gulian(df, length,coin):
    df = HA(df, length)
    # l3Arr1 = ANN(coin,Client.KLINE_INTERVAL_1HOUR)
    # l3Arr2 = ANN(coin,Client.KLINE_INTERVAL_2HOUR)
    l3Arr3 = ANN(coin,Client.KLINE_INTERVAL_4HOUR)
    # canBuy = l3Arr1[48] >= 0.1 and l3Arr2[48] >=0.1 and l3Arr3[48] >=0.1 and l3Arr1[47] < 0.05 #and l3Arr2[47] < l3Arr2[48] and l3Arr3[47] < l3Arr3[48] 
    
    # canSell = (l3Arr1[48] < 0.2 and l3Arr1[47] - l3Arr1[48] > 0.05) or  l3Arr1[48] < 0.05 #or l3Arr2[48] < 0.1 or l3Arr3[48] < 0.1  #and l3Arr3[48] <= 0.05 and l3Arr3[48] <= 0.05

    # if(l3Arr1[48] >= 0.15 and l3Arr2[48] >=0.15 and l3Arr3[48] >=0.15):
    #     print(datetime.datetime.now())
    #     print(coin,l3Arr1[47],l3Arr2[47],l3Arr3[47])
    #     print(coin,l3Arr1[48],l3Arr2[48],l3Arr3[48])
    #     print(getAvg(df, length))
    canSell = False
    canBuy = l3Arr3[48] > 0.1 and l3Arr3[47] < 0.1
    return df,canBuy,canSell
...
def collectData(coin, length):
    ...
    try:
        ...
        if(getAvg(df, length) > 0.6):
            df,canBuy,canSell = gulian(df, length,coin)
            if(canBuy ):#and getPerc(df['close'][length-2],df['open'][length-2]) < 2*getAvg(df, length)
...
def findBestCoin():
    ...
    canBuy = (datetime.datetime.now().hour*60 + datetime.datetime.now().minute )%240 < 6
    # canBuy = True
    ...
    if canBuy:
        ...
        for product in prods:
            ...
            if(True):
                if(float(product['c']) * float(product['cs']) < 300000000):
                    break
...
def balanceAmt(num,profit):
    ...
    for i in range(no_of_coins):
        if(num != i):
            if(profit > 0):
                initialTradeAmount[i] += profit/no_of_coins
                initialTradeAmount[num] -= profit/no_of_coins
            else:
